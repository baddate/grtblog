...
<div class="flex flex-col">
	...
	{:else if error}
		<div class="py-12 text-center font-serif text-xs italic text-error-500">
			{error}
		</div>
	{:else if contents}
		<div class="space-y-8">
			{#if contents.articles.length > 0}
				<div class="space-y-3">
					<div class="flex items-center gap-2 px-1">
						<span class="h-px w-3 bg-accent-500/30"></span>
					...
					<div class="flex flex-col">
						{#each contents.articles as article (article.id)}
							...
							>
								<span
									class="font-serif text-[13px] leading-relaxed text-ink-700 transition-colors group-hover:text-accent-600 dark:text-ink-300 dark:group-hover:text-accent-400"
								>
									{article.title}
								</span>
			...
			{#if contents.moments.length > 0}
				<div class="space-y-3">
					<div class="flex items-center gap-2 px-1">
						<span class="h-px w-3 bg-error-500/30"></span>
						<span class="font-mono text-[9px] font-bold tracking-[0.2em] text-ink-400 uppercase"
							>Moments</span
						>
					</div>
					<div class="flex flex-col">
						{#each contents.moments as moment (moment.id)}
							...
							>
								<span
									class="font-serif text-[13px] leading-relaxed text-ink-700 transition-colors group-hover:text-error-600 dark:text-ink-300 dark:group-hover:text-error-400"
								>
									{moment.title}
								</span>
								<span class="shrink-0 font-mono text-[9px] tracking-tighter text-ink-300">
