...
describe('buildSwitchUrl', () => {
	it('replaces zh-Hans with en in URL path', () => {
		expect(buildSwitchUrl('/zh-Hans/articles/hello/', 'en')).toBe('/en/articles/hello/');
	});

	it('replaces en with zh-Hans (default lang, prefix stripped)', () => {
		expect(buildSwitchUrl('/en/', 'zh-Hans')).toBe('/');
	});

	it('handles root paths with no trailing slash', () => {
		expect(buildSwitchUrl('/zh-Hans', 'en')).toBe('/en/');
	});
});
