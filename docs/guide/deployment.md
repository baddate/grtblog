...
sanblog 使用 Docker Compose 进行一键部署，无需手动配置复杂的运行环境。
...
|------|---------------------|----------|----------|
| Docker Hub | `baddate/` | `baddate/sanblog-server:2.0.2` | 国际通用 |
| GHCR | `ghcr.io/baddate/` | `ghcr.io/baddate/sanblog-server:2.0.2` | 国际通用 |
| CNB | `docker.cnb.cool/baddate/sanblog/` | `docker.cnb.cool/baddate/sanblog/sanblog-server:2.0.2` | 国内服务器推荐 |
...
::: tip 查看可用版本
- GitHub Releases: `https://github.com/baddate/sanblog/releases`
- Docker Hub: `https://hub.docker.com/r/baddate/sanblog-server/tags`
- GHCR: `https://github.com/baddate/sanblog/pkgs/container/sanblog-server`
- CNB: `https://cnb.cool/baddate/sanblog`
:::
...
Nginx 无需重启，会通过 Docker DNS 自动发现新容器。数据库迁移在 server 启动时自动执行。
...
sanblog 内置更新检查功能，会在管理后台自动提示新版本。通过 `.env` 中的 `APP_UPDATE_CHANNEL` 控制检查行为：
...
切换回 stable 通道同理，将值改为 `stable` 后重启。

::: warning 通道与镜像标签配套使用
更新通道只影响**检查和提示**，不会自动升级。收到更新提示后，仍需手动修改 `APP_VERSION` 并拉取新镜像。建议通道与镜像标签保持一致：
...
```bash
docker compose exec postgres pg_dump -U postgres sanblog > backup.sql
```
